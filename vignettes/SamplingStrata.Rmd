...
In particular, the crossover operator ha been modified on the basis of the indications given by @oluing:2019.
...
| *mut_chance*  | Mutation chance, that is for each new individual, the probability to  |
...
| *elitism_rate*  | This  parameter indicates the rate of better solutions that must be 
...
solution <- optimStrata(
  method = "atomic",
	...
	showPlot = FALSE)
```
Note that by so doing the *nStrata* parameter is set equal to the number of atomic strata in each domain . Another possibility is to set a
...
This solution can be given as argument to the parameter *suggestion* in the *optimizeStrata* function, together with the indication of which is the suitable number of strata to be obtained in each domain in the final solution, calculated as:

```{r, eval=TRUE, echo=TRUE, warning=FALSE}
nstrat <- tapply(solutionKmeans1$suggestions, solutionKmeans1$domainvalue,
...
Once the optimal stratification has been obtained, it is possible to select the 
sample from the new version of the frame, taking into account the optimal stratification and allocation:

```{r, eval=TRUE, echo=TRUE}
framenew <- solution$framenew
outstrata <- solution$aggr_strata
sample <- selectSample(framenew, outstrata, writeFiles=FALSE)
```  
...
sample <- selectSample(frame=solution$framenew,outstrata=solution$aggr_strata)
```
