...
* *iter*: indicates the maximum number of iterations (= generations) of the genetic algorithm. Default is 20 
* *pops* The dimension of each generations in terms of individuals. Default is 50. 
...
In the case of the Swiss municipalities, we can use almost all of deault values for parameters with the exception of the errors and strata dataframes, and for the option 'writeFiles':
...
<!-- ![Figure 1](https://github.com/barcaroli/SamplingStrata/tree/master/docs/articles//plotdom3.png) -->
...
the above value, and then selecting all the units individuated by adding the 
selection interval.
...
```{r, eval=FALSE, echo=TRUE}
eval <- evalSolution(framenew, solution$aggr_strata, nsampl=50, writeFiles=TRUE) 
```
...
<!-- ![Figure 2](https://github.com/barcaroli/SamplingStrata/tree/master/docs/articles//cv.png) -->
...
variables Y's are available for the generality of the units in the frame, or at least for a sample of them bay means of which it
...
constraints.  
In order to take into account this problem, and to limit the risk of overestimating the expected precision leveles of the optimized solution, it is possible to carry out the optimization by 
...
The definition and the use of these models is the same that has been implemented in @baillargeon:2014. 
...
In order to take into account the fact that Petal.Width is a proxy of Petal.Length and not the real target variable, we define the following parameters for the linear model linking the two variables:
