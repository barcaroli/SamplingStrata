...
Let the *iris* dataset be the sampling frame. Suppose that the target variable is *Petal.Width*, 
but in the frame we only have *Sepal.Width*. So we declare this latter variable as the Y.
...
frame <- buildFrameDF(iris,
                      ...
                      Y=c("Sepal.Width"),
                      domainvalue = "dom")
strata1 <- buildStrataDF(frame, model = NULL, progress = FALSE)
```

We set a precision constraint equal to 5% and optimize the stratification with regard to *Sepal.Width*:
...
In order to evaluate the expected CV on Petal.Width, we use *evalSolution*:

```{r, eval=TRUE, echo=FALSE, warning=FALSE, }
newstrata <- updateStrata(strata1,solution1)
framenew1 <- updateFrame(frame,newstrata)
framenew1 <- framenew1[order(framenew1$ID),]
framenew1$Y1 <- iris$Sepal.Width
framenew1$Y2 <- iris$Petal.Width
results1 <- evalSolution(framenew1, solution1$aggr_strata, 100, progress = FALSE)
results1$coeff_var
```
While the CV of *Sepal.Width* is compliant with the precision constraint of 5%, the one of *Petal.Width* is far beyond.

In order to cope with this problem, we model the relation between the two variables:
...
and use the model parameters in the construction of the **strata** dataframe:
...
```
If we now evaluate the expected CV's:
```{r, eval=TRUE, echo=TRUE}
newstrata <- updateStrata(strata2,solution2)
framenew2 <- updateFrame(frame,newstrata)
framenew2 <- framenew2[order(framenew2$ID),]
framenew2$Y1 <- iris$Sepal.Width
framenew2$Y2 <- iris$Petal.Width
results2 <- evalSolution(framenew2, solution2$aggr_strata, 500, progress = FALSE)
results2$coeff_var
```

we see that the expected CV of *Petal.Width* has been largely improved. The possibility to fully reach the objective of compliance to the precision constraint depends on the fitting of the model, that in our case is quite low (adjusted R-squared:  0.178).
