...
\section [Analysis of the frame data and manipulation of auxiliary information]{Analysis of the frame data and manipulation of auxiliary information}
...
head(swissframe)
@
\section[Construction of atomic strata and association of the information related to target variables]{Construction of atomic strata and association of the information related to target variables}
...
\section[Choice of the precision constraints for each target estimate]{Choice of the precision constraints for each target estimate}
...
\section[Optimization of frame stratification]{Optimization of frame stratification}
...
\item	{\tt initialStrata}: This is the initial limit on the number of strata for each solution. Default is NA, and in this case it is set equal to the number of atomic strata in each domain.
...
Default is FALSE.
\item	{\tt writeFiles} : iIndicates if the various dataframes and plots produced during the execution have to be written in the working directory. Default is FALSE.
\end{enumerate}
...
@
Note that by so doing the {\tt initialStrata} parameter is set equal to the number of atomic strata in each domain . Another possibility is to set a
...
\section[Initial solution with kmeans clustering of atomic strata]{Initial solution with kmeans clustering of atomic strata}
...
\begin{enumerate}
\item	{\tt minnumstrat}: the minimum number of units to allocated in each stratum(default is 2);
\item	{\tt maxcluster}: the maximum number of clusters to be considered in the execution of kmeans algorithm;
\item	{\tt showPlot}: if TRUE, allows to visualise the optimization.
\end{enumerate}
...
This solution can be given as argument to the parameter {\tt suggestion} in the 'optimizeStrata' parameter.
...
\section[Adjustment of the final sampling size]{Adjustment of the final sampling size}
...
\section[Analysis of results]{Analysis of results}
...
\section[Updating the frame and selecting the sample]{Updating the frame and selecting the sample}
...
\begin{enumerate}
\item	a selection interval is determined by considering the inverse of the
sampling rate in the stratum;
\item	a starting point is determined by selecting a value in this interval;
\item the selection proceeds by selecting as first unit the one corresponding
...
\section[Evaluation of the found solution]{Evaluation of the found solution}
...
expected_cv
@
These values are on average compliant with the precision constraints set (see also Figure ~\ref{graph3}).
\begin{figure}[!hbp]
\centering
\includegraphics[width=0.6\textwidth]{simulation//cv}
\caption{Distribution of the CV's in the different domains for each target variable \label{graph3}}
\end{figure}

Moreover, the estimates of each drawn sample are compared to the known values in the population. 
The distribution of the differences are reported in the boxplots of Figure ~\ref{graph4}. 
It can be seen that the average of the estimates are on average close to the value zero for all the $Y$'s in all domains.
\begin{figure}[!hbp]
\centering
\includegraphics[width=1.000\textwidth]{simulation//differences}
\caption{Distribution of the differences between sample estimates and true values of the parameters in the different domains \label{graph4}}
\end{figure}


\section[Handling 'take-all' strata in the optimization step]{Handling 'take-all' strata in the optimization step}
...
\section[Handling Anticipated Variance]{Handling Anticipated Variance}
...
The definition and the use of these models is the same that has been implemented in \cite{baillargeon:2014}. 
...
Let the \textit{iris} dataset be the sampling frame. Suppose that the target variable is Petal.Lenght, 
